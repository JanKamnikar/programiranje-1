...
# =============================================================================
# Najdaljše naraščajoče podzaporedje -> O(n^2) lažji način

# =============================================================================


def nnpz(zap): #vec resitev vrneš eno tukaj
    @cache
    def f(mesto_v_originalnem_zap, zadnji_el):
        if mesto_v_originalnem_zap >= len(zap):
            return 0,[]
        if zadnji_el > zap[mesto_v_originalnem_zap]:
            return f(mesto_v_originalnem_zap +1, zadnji_el)
        ne_vzamem = f(mesto_v_originalnem_zap +1, zadnji_el)
        vzamem = f(mesto_v_originalnem_zap +1, zap[mesto_v_originalnem_zap])
        if ne_vzamem[0] > vzamem[0] +1:
            return ne_vzamem
        else:
            return vzamem[0] + 1, (vzamem[1] + [zap[mesto_v_originalnem_zap]])#ti se po podproblemih premikaš v levo npr tukaj dodaš [9,8,7,6] + [4]
    return f(0, float("-inf"))[1][::-1]
...
def najdaljse_narascajoce_podzaporedje(zap):
    ...
    # match zap with
    # |x::xs -> 
    # let manj = najdaljse_narascajoce_podzaporedje xs
    # in...


    # najdaljše zap, ki se začne na indexu i, kjer lahko jemljemo samo stevila vecja od z_s
    #a,b,c,...z_s smo na mestu i
    # vrne nam dolžino takega  in še vsa zaporedja, ki temu urezajo (kjer še vedno delamo na enak način)
    @cache
    def inner(zadnja_stevilka,i):
    #tukaj očitno ni več zaporedja
        if i >= len(zap):
            # v tem primeru imamo samo eno zap prazno -> prazno
           return 0, [ [], ] # <-tuple
        if zap[i] >= zadnja_stevilka:
            #vzamemo trenutno stevilko
            vzamemo = inner(zap[i],i+1)
            ne_vzamemo = inner(zadnja_stevilka,i+1)
            if vzamemo[0] >= ne_vzamemo[0]:
                return (vzamemo[0] +1,[
                    [zap[i]] + obstojece # nanj dodamo še trenutno število
                    for obstojece in vzamemo[1] # za vsako najdaljše zaporedje od nas naprej
                ])
            elif vzamemo[0]+1 < ne_vzamemo[0]:
                return ne_vzamemo
            else: #obe sta enako dobri
                    return (ne_vzamemo[0],
                    ne_vzamemo[1] +
                    [
                        [zap[i]]+ obstojece
                        for obstojece in vzamemo[1]
                    ]
                )   
        else:
            #ne
            return inner(zadnja_stevilka,i+1)
            
    #lahko bi vzeli -\infty
    if not zap:
        return [[]]
    return inner(min(zap),0)[1]
...
#O(n^2)
def zabica(mocvara):
    #če smo na lokvanju index pristali z e energije potrebujemo inner(index,e) korakov
    @cache
    def inner(index, e):
        if index >= len(mocvara):
            return 0
        # lahko skocimo na vse indexe do [index +1, index +e + mocvara[index]]
        mozne_resitve = [
            1 + inner(index + i,e + mocvara[index]- i) for i in range(1,e + mocvara[index]) 
        ]
        return min(mozne_resitve) 
    return inner(0,0)
